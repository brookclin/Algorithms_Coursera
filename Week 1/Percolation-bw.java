...
public class Percolation {
   ...
   public Percolation(int n)               
   {
     ...
     uf_backwash = new WeightedQuickUnionUF(n*(n+1)+1);
     openSite = new boolean[n*(n+2)+2];
     for (int i = 1; i <= n; i++)
     {
       uf.union(xyTo1D(0, 1), xyTo1D(0, i));
       uf_backwash.union(xyTo1D(0, 1), xyTo1D(0, i));
   ...
   public void open(int i, int j)          
   {
     ...
     if (openSite[xyTo1D(i-1, j)])
     {
       ...
       uf_backwash.union(xy, xyTo1D(i-1, j));
     }
     if (openSite[xyTo1D(i+1, j)])
     {
       uf.union(xy, xyTo1D(i+1, j));
     ...
     if (j != 1 && openSite[xyTo1D(i, j-1)])
     {
       ...
       uf_backwash.union(xy, xyTo1D(i, j-1));
     }
   ...
   {
     isValid(i, j);
     return isOpen(i, j) && uf_backwash.connected(xyTo1D(i, j), xyTo1D(0, 1));
   }
